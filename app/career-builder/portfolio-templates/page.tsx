...
interface PortfolioTemplateItem {
    id: string;
    name: string;
    industry: string;
...
export default function PortfolioTemplatesPublicPage() {
    const [templates, setTemplates] = useState<PortfolioTemplateItem[]>([]);
    ...
    useEffect(() => {
        const loadTemplates = async () => {
            ...
            try {
                ...
                if (data.success) {
                    const activePublic = (data.templates || []).filter(
                        (t: PortfolioTemplateItem) => t.status === 'Active' && t.visibility === 'Public'
                    );
                    setTemplates(activePublic);
                }
            } catch (err) {
                console.error('Error fetching portfolio templates:', err);
            } finally {
                setLoading(false);
            }
        };
    ...
    const filteredTemplates = templates.filter(t => {
        ...
        const matchesPrice = priceFilter === 'All' || t.pricing === priceFilter;
            ...
            <div className="relative overflow-hidden bg-black select-none text-white min-h-screen">
                ...
                <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl relative z-10 pt-8 pb-20">
                    ...
                    <nav className="flex items-center gap-2 mb-12 text-xs font-semibold uppercase tracking-wider text-zinc-500">
                        ...
                        <ChevronRight className="w-3.5 h-3.5 text-zinc-700" />
                        <span className="text-[#FF7A00]">Portfolio Templates</span>
                    </nav>
                    ...
                    <section className="text-center py-16 md:py-20 max-w-3xl mx-auto space-y-6 animate-fade-in-scale">
                        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-[#FF6A00]/20 bg-[#FF6A00]/5 text-xs font-medium text-[#FF8A33] tracking-wider uppercase">
                            ...
                            <span>Recruiter Showcase Layouts</span>
                        </div>
                        <h1 className="text-4xl sm:text-6xl font-bold tracking-tight font-heading leading-none">
                            Portfolio <span className="gradient-text">Templates</span>
                        </h1>
                        <p className="text-md sm:text-lg text-zinc-400 max-w-2xl mx-auto leading-relaxed">
                            Spotlight your projects and case studies. Select customizable web layouts designed to deploy directly to personal domains.
                        </p>
                    </section>

                    {/* Search & Filters */}
                    <div className="p-6 rounded-2xl bg-white/[0.01] border border-[#FF7A00]/10 backdrop-blur-md space-y-4 mb-10">
                        <div className="flex flex-col md:flex-row gap-4">
                            <div className="relative flex-1">
                                <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-500" />
                                <input
                                    type="text"
                                    placeholder="Search portfolios by style..."
                                    value={searchQuery}
                    ...
                    {loading ? (
                        <div className="py-20 text-center">
                            ...
                            <p className="text-zinc-500 text-xs">Loading portfolios...</p>
                        </div>
                        ...
                        <div className="p-16 rounded-2xl bg-white/[0.01] border border-white/[0.04] text-center text-zinc-500 text-xs max-w-lg mx-auto">
                            ...
                            No portfolio layouts found matching search options.
                        </div>
                    ) : (
                            ...
                            {filteredTemplates.map((row) => (
                                <div key={row.id} className="phoenix-card p-6 flex flex-col justify-between items-start space-y-6 relative border border-white/[0.04] hover:border-[#FF7A00]/20 transition-all duration-300">
                                    <div className="space-y-4 w-full">
                                        <div className={`w-full aspect-[4/3] rounded-xl bg-gradient-to-br ${row.color || 'from-zinc-900 to-zinc-950'} border border-white/[0.08] relative overflow-hidden flex flex-col p-4 justify-between group shadow-xl`}>
                                            {row.thumbnail ? (
                                                <img src={row.thumbnail} alt={row.name} className="absolute inset-0 w-full h-full object-cover" />
                                            ) : (
                                                <>
                                                    <div className="flex justify-between items-start">
                                                        ...
                                                        {row.pricing === 'Premium' && (
                                                            <span className="text-[7px] bg-yellow-500/10 text-yellow-400 border border-yellow-500/20 font-bold px-1 rounded flex items-center gap-0.5">
                                                                <Lock className="w-2 h-2" /> PREMIUM
                                                            </span>
                                                        )}
                                                    </div>
                                                    <div className="space-y-2 text-left">
                                                        <div className="w-16 h-2 bg-[#FF7A00]/20 rounded" />
                                                        <div className="w-28 h-3.5 bg-white/15 rounded" />
                                                    </div>
                                                </>
                                            )}
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <p className="text-[10px] text-zinc-500 uppercase tracking-widest font-bold mt-0.5">{row.industry} • {row.pricing}</p>
                                        </div>
                                        ...
                                        {row.features && row.features.length > 0 && (
                                            <div className="flex flex-wrap gap-2 pt-2 border-t border-white/[0.04]">
                                                {row.features.map((feat, idx) => (
                                                    <span key={idx} className="text-[8px] bg-zinc-500/10 text-zinc-400 border border-zinc-500/20 font-bold px-1.5 py-0.5 rounded tracking-wider">
                                                        {feat.toUpperCase()}
                                                    </span>
                                                ))}
                                            </div>
