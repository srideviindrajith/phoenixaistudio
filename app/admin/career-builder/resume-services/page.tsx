...
export default function ResumeServicesCRUD() {
    const [services, setServices] = useState<ResumeServiceItem[]>([])
    const [loading, setLoading] = useState(true)

    const fetchServices = useCallback(async () => {
        setLoading(true)
        try {
            const res = await fetch('/api/resume-services')
            const data = await res.json()
            setServices(data.services || [])
        } catch (error) {
            console.error('Error fetching resume services:', error)
        } finally {
            setLoading(false)
        }
    }, [])
    ...
    const [editingSvc, setEditingSvc] = useState<ResumeServiceItem | null>(null);
    ...
    const stats = [
        ...
        { title: 'Average Cost', value: '₹2,999', icon: TrendingUp }
    ];
    ...
    const handleSave = (e: React.FormEvent) => {
        ...
        if (editingSvc) {
            ...
            const newItem: ResumeServiceItem = {
    ...
    const openEditModal = (svc: ResumeServiceItem) => {
        setEditingSvc(svc);
        setFormState({
            title: svc.title,
            desc: svc.desc,
    ...
    return (
        <div className="max-w-7xl space-y-6 lg:space-y-8 select-none text-white animate-fade-in">
            {/* Header */}
            <div>
                <h1 className="font-heading text-2xl font-bold text-white md:text-3xl">Resume Services Manager</h1>
                <p className="text-gray-400 text-sm mt-1">Configure pricing tiers, SLA deliveries, and description copy for Resume Services.</p>
            </div>
                        ...
                        <form onSubmit={handleSave} className="space-y-4 text-xs text-zinc-300">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    placeholder="e.g. 2-8 years of experience"
                                    className="w-full h-11 px-4 rounded-xl border border-white/[0.08] bg-black text-white focus:outline-none focus:border-[#FF7A00]"
                                />
                            </div>
                            <div>
